...
job_args = getResolvedOptions(
    sys.argv, ["glue_database_name", "output_bucket_name", "TempDir"]
)
...
covid_hiring_dyf = glueContext.create_dynamic_frame.from_catalog(
    ...
    transformation_ctx="covid_hiring_dyf",
)

# write data from S3 into Redshift
redshift_load_dyf = glueContext.write_dynamic_frame.from_jdbc_conf(
    frame=covid_hiring_dyf,
    catalog_connection="redshift-connect",
    connection_options={
        "preactions": "truncate table covid_hiring_table;",
        "dbtable": "covid_hiring_table",
        "database": "db-covid-hiring",
    },
    redshift_tmp_dir=tempDir,
